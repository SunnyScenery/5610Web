const student_name = "Linsong Wang";
...
function findBiggestNumber(map) {
  ...
  // filter the nulls and undefined
  const filteredMap = map.map((row) =>
    row.filter((item) => item !== null && item !== undefined)
  );
  // flatten the array
  const flattenedMap = filteredMap.flat();
  // find the biggest number
  if (flattenedMap.length === 0) return undefined;
  const biggestNumber = Math.max(...flattenedMap);
...
function balancedString(str) {
  /*
    ...
    */
   // create a hashmap
   const buildstr = "abcdefghijklmnopqrstuvwxyz";
   const map = {};
   Array.from(buildstr).forEach(element => {
    map[element] = 0;
   });
    ...
    // check if the number of all the characters in the string is the same
    const values = Object.values(map);
    let count = 0;
    for (const element of values.filter(x => x != 0)) {
      if (count == 0) {
        count = element;
      }
      if (count != element) {
